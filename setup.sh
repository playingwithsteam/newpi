#!/bin/sh
pause -p "[ENTER] to continue: Updating the software database and install a little tool... (wget)"
# First: Get ready
apt-get update; apt-get -y install wget
# Then speed things up
pause -p "[ENTER] to continue: Let's back up the main RasPi config and replace it."
...
pause -p "[ENTER] to continue: Now overscan is off, card reader is overlocked. Downloading other scripts."
...
pause -p "[ENTER] to continue: Time to reboot. Execute /root/setup/runonce.sh after, to get fully updated and then reboot again"
sudo reboot
